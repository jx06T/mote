...
- **寫作作品庫、專屬評析與編輯器約束**：
  ...
  - 雙軌寫作作品彙整（`UnifiedWritingItem`）：完整收納「電子寫作」與「紙本模擬考（手寫 OCR 辨識稿與評析）」所有歷史作品。
  - 提供獨立作品庫檢視頁（`/essays`）與編輯器頂部快速切換抽屜（`EssayListDrawer`），支援多來源（電子寫作 / 紙本模考）與狀態篩選。
  - **單一作品專屬檢視與評析頁（`/essays/:id` - `EssayDetailPage`）**：
    - 支援原文閱讀（紙質排版、手寫照片）、八維 AI 評析報告與雙欄對照（Split View）。
  - **全域弱點追蹤頁（`/analysis` - `AnalysisPage`）**：
    - 聚焦宏觀常態弱點統計與歷次作品索引，與單篇專屬評析明確區隔。
  - 即時編輯與歷史刪除行為分流處理。
